...
void main() {
    ...
    // https://github.com/chrischristakis/Winston-Shield/blob/master/shaders/bubble.fs
	
    // - Rim lighting -
    vec3 view_direction = normalize(fragment_in.TangentViewPos - fragment_in.TangentFragPos);
	vec3 view_angle = normalize(-view_direction);
	...
	float rimStrength = 1 - abs(dot(view_angle, normal)); // The more orthogonal, the stronger

	float rimFactor = pow(rimStrength, 4); // higher power = sharper rim light
	vec4 rim = vec4(rimFactor);

    float scene_depth = LouronLineariseDepth(LouronSampleDepthTexture(gl_FragCoord.xy));
    float bubble_depth = LouronLineariseDepth(gl_FragCoord.z);
    
	float distance = abs(bubble_depth - scene_depth); // linear difference in depth 
    ...
    out_FragColour = vec4(u_MaterialUniforms.BubbleColour) + intersection + rim;
}
