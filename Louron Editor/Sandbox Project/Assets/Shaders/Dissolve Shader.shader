...
// If you do not wish to have Material Uniforms, then remove this entire block.
...
void main() {
    ...
    vec3 result;
    result = LouronCalculatePBR_Lighting_All(normal, fragment_in.TangentFragPos, view_direction, albedo, metallic, roughness);
    result = LouronCalculateHDR_Exposure(result, 5.0);
    result = LouronCalculateGammaCorrection(result, 2.2);

    // Apply the glow color only around the edge where the dissolve is happening
    // Ensure that we only apply the glow where edgeGlow is significant (0 < edgeGlow < 1)
    result = mix(result, u_MaterialUniforms.GlowColour, edgeGlow);  // Blend between the lighting result and the glow color
