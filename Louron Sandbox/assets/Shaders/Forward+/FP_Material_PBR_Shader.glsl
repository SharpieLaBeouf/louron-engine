...
vec3 CalcSpotLights(vec3 normal, vec3 fragPos, vec3 viewDir){
    ...
    for (int i = 0; i < SL_Buffer_Data.data.length() && SL_Buffer_Data.data[i].lastLight == false; ++i) {
       ...
       // diffuse shading
        ...
        // combine results
        vec3 ambient = light.ambient.xyz * vec3(texture(u_Material.diffuseMap, TexCoord));
        vec3 diffuse = light.diffuse.xyz * diff * vec3(texture(u_Material.diffuseMap, TexCoord));
        vec3 specular = light.specular.xyz * spec * vec3(texture(u_Material.specularMap, TexCoord));
